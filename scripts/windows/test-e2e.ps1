...
try {
  ...
  if (-not $SkipDesktop) {
    ...
    if (-not (Invoke-AlfredHost "click" @{ x = $x; y = $y } "focus Notepad editor" "Editor").ok) { throw "Pointer input failed." }
    if (-not (Invoke-AlfredHost "typeText" @{ text = "Alfred Windows end-to-end smoke test" } "type smoke-test text" "Notepad editor").ok) { throw "Keyboard input failed." }
    Write-Host "PASS application discovery, UI Automation, capture, pointer, and keyboard"
