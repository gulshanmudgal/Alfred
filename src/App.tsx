...
function SettingsView({ settings, providers, system, onSave }: { settings: AppSettings; providers: ProviderStatus[]; system: SystemInfo; onSave: (settings: AppSettings) => void }) {
  ...
  return <div className="page settings-page"><section className="page-title"><div><span className="eyebrow">PREFERENCES</span><h1>Settings</h1><p>Configure engines, credentials, computer access, and local storage without editing files.</p></div></section><div className="settings-layout">
    ...
    <section className="settings-section"><h2>Diagnostic logs</h2><p>Write a local JSONL file for each planner turn and tool call. Nothing is sent to a provider. Turn this on when a run fails and you want a LangSmith-style trace.</p><label className="toggle-row"><input type="checkbox" checked={draft.diagnosticLogging} onChange={(event) => setDraft({ ...draft, diagnosticLogging: event.target.checked })}/><span>Record planner and tool-call logs on this machine</span></label><button className="secondary-button" onClick={async () => { try { const folder = await invoke<string>("run_logs_folder"); setMessage(`Logs are in ${folder}`); } catch (caught) { setMessage(String(caught)); } }}>Show log folder</button></section>
